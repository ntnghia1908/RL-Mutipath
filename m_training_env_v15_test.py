...
# BW_TRACE1 = 'markovian_bitrate_2'
# BW_TRACE2 = 'markovian_bitrate_2'
...
BUFFER_THRESH = 34.0  # sec, max buffer limit
...
class Env():
    ...
    def reset(self):
        ...
        delay = self.down_time(segment_size, self.cur_time, PATH1)
    ...
    def step(self, action):
        ...
        # NEW STEP
        cur_down_time = self.event[0][0]
        cur_down_path = self.event[0][4]
        ...
        self.event = np.vstack((self.event, [[cur_down_time + delay, DOWNF, down_id, down_quality, cur_down_path]]))
        self.event = np.delete(self.event, 0, 0)  # remove the current considering event from event

        self.event = self.event[self.event[:, 0].argsort()]

        if cur_down_path == PATH1:
            self.est_throughput1 = segment_size / delay     # in bits per second
        if cur_down_path == PATH2:
        ...
        for i in range(self.play_id, self.play_id + self.SEGMENT_SPACE):
            ...
            #         chunk_state = np.append(chunk_state, 1)
                chunk_state = np.append(chunk_state, self.down_segment[i])
                ...
                chunk_state = np.append(chunk_state, 6)
        ...
        self.state = np.append(self.state, chunk_state)  # which quality levels of chunks in buffer
        ...
        # cur_down_path = int(self.event[0][4])
        return self.state, reward, self.end_of_video, line, self.play_id, sum_reward, cur_down_path, self.down_segment
