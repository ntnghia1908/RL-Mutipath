...
def main():
    ...
    qua_rw = np.asarray(qua_rw).mean()
    smth_rw = np.asarray(smth_rw).mean()
    rebuffering_rw = np.asarray(rebuffering_rw).mean()
    total_rewards = np.asarray(total_rewards).mean()
    ...
    return qua_rw, smth_rw, rebuffering_rw, total_rewards

if __name__ == '__main__':
    print('seedNo', 'reward', 'quality_reward', 'smooth_reward', 'buffering_reward')
    for n in range(100):
        np.random.seed(n)
        q, s, rb, rw = main()
        print(n, q, s, rb, rw)
