import numpy as np
import math
from simulator.get_down_size import video_list_collector

VIDEO_CHUNCK_LEN = 1                            # sec, every time add this amount to buffer
BITRATE_LEVELS = 7
...
class VideoPlayer():
    ...
    def reset(self, mode = 'train'):
        if mode == 'test':
            ...
            self.video_interator = 0
        else:
            self.net_seg_iterator = np.random.randint(0, len(self.bitrate_list)-1) 
            self.video_interator = np.random.randint(0, len(self.video_list[0])-1)

        self.video_seg_download = 0
        self.seg_time_stamp = 0
        self.buffer = 0
        return self.video_list[:,self.video_interator] 
    ...
    def download(self, bitrate_level):
        ...
        self.seg_time_stamp -= pass_seg

        #download video segment in bytes
        segment = float(self.video_list[bitrate_level][self.video_interator])          
        ...
        self.video_interator = (self.video_interator + 1) % len(self.video_list[0]) #loop back if finished video
        terminate = False

        if self.video_seg_download >= TOTAL_VIDEO_CHUNCK:                       #if terminate reset
            terminate = True
            self.reset()
        next_segments = self.video_list[:,self.video_interator]                 #get sizes of next download options
