...
def play_video(env, TrainNet, TargetNet, epsilon, copy_step):
    ...
    write2csv('m_evaluate/play_event{}'.format(round(total_reward,1)), env.play_event)
    write2csv('m_evaluate/down_event{}'.format(round(total_reward,1)), env.down_event)
...
def main():
    ...
    epoch = []
    avg_rewards = []
    file_name = 'lr{}_decay{}_fixtrace_v14'.format(lr, decay)
    ...
    PRETRAIN = False
    if PRETRAIN == True:
        file_name = 'lr5e-05_decay0.995_markov_v14.h5'
        epsilon = 0.99
        decay = 0.9
        min_epsilon = 0.1
        TrainNet.load_model(np.array([state], dtype='float32'), 'model/2DQNmodel_{}'.format(file_name))
        TargetNet.load_model(np.array([state], dtype='float32'), 'model/2DQNmodel_{}'.format(file_name))
        file_name = file_name + 'pretrain_'

    TEST = True
    if TEST == True:
        file_name = 'lr45_decay999_batch200_fixtrace_v14'
        epsilon = 0.0
        decay = 1.0
        min_epsilon = 0.0
        N = 100
